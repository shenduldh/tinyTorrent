import { log } from '../dependences/index.ts';
import { PieceResult, PieceWork, Torrent, Peer } from '../types/index.ts';
import { Worker } from './worker.ts';
import { Buffer } from '../dependences/index.ts';
...
export class Downloader {
	...
	torrent: Torrent;
	peers: Peer[];
	workQueue: PieceWork[]; // piece index
	result: PieceResult[] = [];
	workingWorkers = 0;
	...
	async start() {
		...
		const onResult = (r: PieceResult) => {
			...
			const percent = (
				(this.result.length / this.torrent.pieceCount) *
				100
			).toFixed(2);
