import { assert, createHash, Buffer } from '../dependences/index.ts';
import { MSG_ID } from './downloader.ts';
import { Message, Peer } from '../types/index.ts';
...
export const queryEncodeArrayBuffer = (ab: ArrayBuffer): string =>
...
export const genRandomStr = (length: number): string =>
	new Array(length)
		.fill(0)
		.map(_x => alphabet[Math.floor(Math.random() * alphabet.length)])
		.join('');

export const pick = <O extends object>(
	obj: O,
	keys: (keyof O)[]
	...
	return new Promise((resolve, reject) => {
		...
		Deno.connect(options)
			.then(conn => {
				...
				resolved = true;
				// if connected successfully, return the connection
				resolve(conn);
			})
			.catch(_ => {
				// discard
