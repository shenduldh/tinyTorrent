...
import { assert, log, Buffer } from '../dependences/index.ts';
import { MAX_BLOCK_SIZE, MAX_PENDING_REQUEST, MSG_ID } from './downloader.ts';
import {
	BitTorrent,
	connectWithTimeout,
	readFull,
...
import { Torrent, Peer, PieceResult, PieceWork, Message } from '../types/index.ts';
		...
		while (downloaded < work.length) {
			...
			if (!this.choked) {
				while (pendingCount < MAX_PENDING_REQUEST && requested < work.length) {
					...
					this.sendBlockRequest(pieceIndex, requested, blockSize).catch(e => {
						err = e;
					});
			...
			switch (msg.id) {
				...
				case MSG_ID.Piece:
					{
						const { content, offset } = BitTorrent.parsePieceMessage(
							work.pieceIndex,
							work.length,
							msg
						);
